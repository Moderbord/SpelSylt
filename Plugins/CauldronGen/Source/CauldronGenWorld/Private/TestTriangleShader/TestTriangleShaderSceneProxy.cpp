...
class FTestTriangleShaderRendererExtension : public FRenderResource
{
public:
	FTestTriangleShaderRendererExtension()
		: bInFrame(false)
		, DiscardId(0)
	{}

	virtual ~FTestTriangleShaderRendererExtension()
	{}
	...
	TestTriangleShaderMesh::FDrawInstanceBuffers& AddWork(FTestTriangleShaderSceneProxy const* InProxy, FSceneView const* InMainView, FSceneView const* InCullView);
...
private:
	...
	TArray<TestTriangleShaderMesh::FDrawInstanceBuffers> Buffers;
	...
	TArray<FTestTriangleShaderSceneProxy const*> SceneProxies;
...
TGlobalResource< FTestTriangleShaderRendererExtension > GTestTriangleShaderRendererExtension;

void FTestTriangleShaderRendererExtension::RegisterExtension()
{
	static bool bInit = false;
	if (!bInit)
	{
		GEngine->GetPreRenderDelegateEx().AddRaw(this, &FTestTriangleShaderRendererExtension::BeginFrame);
		GEngine->GetPostRenderDelegateEx().AddRaw(this, &FTestTriangleShaderRendererExtension::EndFrame);
		bInit = true;
	}
}

void FTestTriangleShaderRendererExtension::ReleaseRHI()
{
	Buffers.Empty();
}

TestTriangleShaderMesh::FDrawInstanceBuffers& FTestTriangleShaderRendererExtension::AddWork(FTestTriangleShaderSceneProxy const* InProxy, FSceneView const* InMainView, FSceneView const* InCullView)
{
	...
	if (WorkDesc.BufferIndex == -1)
	{
		...
		WorkDescs.Add(WorkDesc);
		TestTriangleShaderMesh::InitializeInstanceBuffers(GetImmediateCommandList_ForRenderCommand(), Buffers[WorkDesc.BufferIndex]);
	}

	return Buffers[WorkDesc.BufferIndex];
}

void FTestTriangleShaderRendererExtension::BeginFrame(FRDGBuilder& GraphBuilder)
{
	// If we hit this then BeginFrame()/EndFrame() logic needs fixing in the Scene Renderer.
	if (!ensure(!bInFrame))
	{
...
void FTestTriangleShaderRendererExtension::EndFrame()
{
	ensure(bInFrame);
	bInFrame = false;
	...
	for (int32 Index = 0; Index < DiscardIds.Num();)
	{
		...
		{
			TestTriangleShaderMesh::ReleaseInstanceBuffers(Buffers[Index]);
...
void FTestTriangleShaderRendererExtension::EndFrame(FRDGBuilder& GraphBuilder)
{
	EndFrame();
}

const static FName NAME_TestTriangleShader(TEXT("TestTriangleShader"));

FTestTriangleShaderSceneProxy::FTestTriangleShaderSceneProxy(UTestTriangleShaderComponent* InComponent)
	: FPrimitiveSceneProxy(InComponent, NAME_TestTriangleShader)
	, VertexFactory(nullptr)
{
	GTestTriangleShaderRendererExtension.RegisterExtension();
...
SIZE_T FTestTriangleShaderSceneProxy::GetTypeHash() const
{
	static size_t UniquePointer;
	return reinterpret_cast<size_t>(&UniquePointer);
}

uint32 FTestTriangleShaderSceneProxy::GetMemoryFootprint() const
{
	return(sizeof(*this) + FPrimitiveSceneProxy::GetAllocatedSize());
}

void FTestTriangleShaderSceneProxy::OnTransformChanged()
{
	// TODO
...
void FTestTriangleShaderSceneProxy::CreateRenderThreadResources()
{
	// Gather vertex factory uniform parameters.
	FTestTriangleShaderParameters UniformParams;
	// TODO UNIFORM INIT

	// Create vertex factory.
	VertexFactory = new FTestTriangleShaderVertexFactory(GetScene().GetFeatureLevel(), UniformParams);
	VertexFactory->InitResource();
}

void FTestTriangleShaderSceneProxy::DestroyRenderThreadResources()
{
...
FPrimitiveViewRelevance FTestTriangleShaderSceneProxy::GetViewRelevance(const FSceneView* View) const
{
...
void FTestTriangleShaderSceneProxy::GetDynamicMeshElements(const TArray<const FSceneView*>& Views, const FSceneViewFamily& ViewFamily, uint32 VisibilityMap, FMeshElementCollector& Collector) const
{
	check(IsInRenderingThread());

	if (GTestTriangleShaderRendererExtension.IsInFrame())
	{
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
	{
		if (VisibilityMap & (1 << ViewIndex))
		{
			...
			TestTriangleShaderMesh::FDrawInstanceBuffers& Buffers = GTestTriangleShaderRendererExtension.AddWork(this, ViewFamily.Views[0], Views[ViewIndex]);
			...
			Mesh.Elements.SetNumZeroed(1);
			{
				...
				FTestTriangleShaderUserData* UserData = &Collector.AllocateOneFrameResource<FTestTriangleShaderUserData>();
...
// bool FTestTriangleShaderSceneProxy::HasSubprimitiveOcclusionQueries() const
// {
// 	return false;
// }

// const TArray<FBoxSphereBounds>* FTestTriangleShaderSceneProxy::GetOcclusionQueries(const FSceneView* View) const
// {
// 	// return &DefaultOcclusionVolumes;
// }

// void FTestTriangleShaderSceneProxy::BuildOcclusionVolumes(TArrayView<FVector2D> const& InMinMaxData, FIntPoint const& InMinMaxSize, TArrayView<int32> const& InMinMaxMips, int32 InNumLods)
// {
// 	// TODO
// }

// void FTestTriangleShaderSceneProxy::AcceptOcclusionResults(FSceneView const* View, TArray<bool>* Results, int32 ResultsStart, int32 NumResults)
// {
// 	check(IsInRenderingThread());
...
namespace TestTriangleShaderMesh
{
	...
	struct FTestTriangleShaderRenderInstance {
		float Position[3];
	};
	...
	IMPLEMENT_GLOBAL_SHADER(FInitBuffersVHM_CS, "/CauldronGenWorldShaders/TestTriangleShader/TestTriangleShaderCompute.usf", "InitBuffersCS", SF_Compute);
	...
	IMPLEMENT_GLOBAL_SHADER(FCollectQuadsVHM_CS, "/CauldronGenWorldShaders/TestTriangleShader/TestTriangleShaderCompute.usf", "CollectQuadsCS", SF_Compute);
	...
	IMPLEMENT_GLOBAL_SHADER(FInitInstanceBufferVHM_CS, "/CauldronGenWorldShaders/TestTriangleShader/TestTriangleShaderCompute.usf", "InitInstanceBufferCS", SF_Compute);
	...
	public:
		...
		BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
			...
			SHADER_PARAMETER_UAV(RWStructuredBuffer<TestTriangleShaderMesh::FTestTriangleShaderRenderInstance>, RWInstanceBuffer)
			SHADER_PARAMETER_UAV(RWBuffer<uint>, RWIndirectArgsBuffer)
			RDG_BUFFER_ACCESS(IndirectArgsBuffer, ERHIAccess::IndirectArgs)
		END_SHADER_PARAMETER_STRUCT()
	};

	IMPLEMENT_GLOBAL_SHADER(FCullInstancesVHM_CS, "/CauldronGenWorldShaders/TestTriangleShader/TestTriangleShaderCompute.usf", "CullInstancesCS", SF_Compute);
	...
	{
		{
			FRHIResourceCreateInfo CreateInfo(TEXT("FTestTriangleShader.InstanceBuffer"));
			const int32 InstanceSize = sizeof(TestTriangleShaderMesh::FTestTriangleShaderRenderInstance);
		...
		{
			FRHIResourceCreateInfo CreateInfo(TEXT("FTestTriangleShader.InstanceIndirectArgsBuffer"));
	...
	{
		OutResources.QueueInfo = GraphBuilder.CreateBuffer(FRDGBufferDesc::CreateStructuredDesc(sizeof(WorkerQueueInfo), 1), TEXT("TestTriangleShaderMesh.QueueInfo"));
		OutResources.QueueInfoUAV = GraphBuilder.CreateUAV(OutResources.QueueInfo);
		OutResources.QueueBuffer = GraphBuilder.CreateBuffer(FRDGBufferDesc::CreateBufferDesc(sizeof(uint32), InDesc.MaxPersistentQueueItems), TEXT("TestTriangleShaderMesh.QuadQueue"));
		OutResources.QueueBufferUAV = GraphBuilder.CreateUAV(FRDGBufferUAVDesc(OutResources.QueueBuffer, PF_R32_UINT));

		OutResources.QuadBuffer = GraphBuilder.CreateBuffer(FRDGBufferDesc::CreateBufferDesc(sizeof(uint32) * 2, InDesc.MaxRenderItems), TEXT("TestTriangleShaderMesh.QuadBuffer"));
		...
		OutResources.FeedbackBuffer = GraphBuilder.CreateBuffer(FeedbackBufferDesc, TEXT("TestTriangleShaderMesh.FeedbackBuffer"));
		OutResources.FeedbackBufferUAV = GraphBuilder.CreateUAV(FRDGBufferUAVDesc(OutResources.FeedbackBuffer, PF_R32_UINT));

		OutResources.IndirectArgsBuffer = GraphBuilder.CreateBuffer(FRDGBufferDesc::CreateIndirectDesc(IndirectArgsByteSize), TEXT("TestTriangleShaderMesh.IndirectArgsBuffer"));
	...
	void AddPass_TransitionAllDrawBuffers(FRDGBuilder& GraphBuilder, TArray<TestTriangleShaderMesh::FDrawInstanceBuffers> const& Buffers, TArrayView<int32> const& BufferIndices, bool bToWrite)
	{
	...
	void AddPass_CullInstances(FRDGBuilder& GraphBuilder, FGlobalShaderMap* InGlobalShaderMap, FProxyDesc const& InDesc, FVolatileResources& InVolatileResources, FDrawInstanceBuffers& InOutputResources, FChildViewDesc const& InViewDesc)
	{
		...
		int32 IndirectArgOffset = TestTriangleShaderMesh::IndirectArgsByteOffset_FinalCull;
...
void FTestTriangleShaderRendererExtension::SubmitWork(FRDGBuilder& GraphBuilder)
{
	// SCOPE_CYCLE_COUNTER(STAT_TestTriangleShaderMesh_SubmitWork);
	// DECLARE_GPU_STAT(TestTriangleShaderMesh)
	// RDG_EVENT_SCOPE(GraphBuilder, "TestTriangleShader");
	// RDG_GPU_STAT_SCOPE(GraphBuilder, TestTriangleShaderMesh);
	...
	while (WorkIndex < NumWorkItems)
	{
		...
		FTestTriangleShaderSceneProxy const* Proxy = SceneProxies[WorkDescs[WorkIndex].ProxyIndex];
		...
		TestTriangleShaderMesh::FProxyDesc ProxyDesc;
		...
		while (WorkIndex < NumWorkItems && SceneProxies[WorkDescs[WorkIndex].ProxyIndex] == Proxy)
		{
			...
			TestTriangleShaderMesh::FViewData MainViewData;
			TestTriangleShaderMesh::GetViewData(MainView, MainViewData);

			TestTriangleShaderMesh::FMainViewDesc MainViewDesc;
			...
			// 	Plane = TestTriangleShaderMesh::TransformPlane(Plane, Proxy->WorldToUV, Proxy->WorldToUVTransposeAdjoint);
			// 	MainViewDesc.Planes[PlaneIndex] = TestTriangleShaderMesh::ConvertPlane(Plane);
			// }
			...
			TestTriangleShaderMesh::FVolatileResources VolatileResources;
			TestTriangleShaderMesh::InitializeResources(GraphBuilder, ProxyDesc, MainViewDesc, VolatileResources);

			// Build graph
			TestTriangleShaderMesh::AddPass_InitBuffers(GraphBuilder, GetGlobalShaderMap(GMaxRHIFeatureLevel), ProxyDesc, VolatileResources);
			TestTriangleShaderMesh::AddPass_CollectQuads(GraphBuilder, GetGlobalShaderMap(GMaxRHIFeatureLevel), ProxyDesc, VolatileResources, MainViewDesc);
			...
			while (WorkIndex < NumWorkItems && MainViews[WorkDescs[WorkIndex].MainViewIndex] == MainView)
			{
				...
				TestTriangleShaderMesh::FChildViewDesc ChildViewDesc;
				...
				// Build graph
				TestTriangleShaderMesh::AddPass_CullInstances(GraphBuilder, GetGlobalShaderMap(GMaxRHIFeatureLevel), ProxyDesc, VolatileResources, Buffers[WorkDescs[WorkIndex].BufferIndex], ChildViewDesc);
